...
Allocates train as max_tokens minus the validation reserve (--max-val-tokens), and validation as
min(20%% of max_tokens, that cap), then packs into train/val memmaps.
...
def preprocess(example, tokenizer: Tokenizer):
    ...
    # --- MetaMathQA-style QA (query / response) ---
    elif "query" in example and "response" in example:
        q, a = str(example["query"]), str(example["response"])
    ...
    # --- Alpaca format (output or response, e.g. Magicoder-Evol-Instruct) ---
    elif "instruction" in example:
        instr, inp = str(example["instruction"]), str(example.get("input", ""))
        out = str(example.get("output") or example.get("response", ""))
        prompt = f"<|user|>\n{instr}\n{inp}\n" if inp else f"<|user|>\n{instr}\n"
        p_tokens = tokenizer.encode(prompt, add_special_tokens=False)
        input_ids += p_tokens
        label_mask += [False] * len(p_tokens)
...
def main(opts) -> None:
    ...
    print(f"Total available tokens: {total_available}")
    ...
    # Train budget: all of max_tokens except a fixed reserve for val (not 80%%).
    # Val budget: 20%% of max_tokens, capped by --max-val-tokens and availability.
    train_target = min(max(0, max_tokens - opts.max_val_tokens), train_total_tokens)
    val_cap = int(max_tokens * 0.2)
    val_target = (
        min(val_cap, val_total_tokens, opts.max_val_tokens)
        if val_dataset is not None
        else min(val_cap, max(0, train_total_tokens - train_target), opts.max_val_tokens)
    )
    ...
    log.info(
        f"Train/val targets (20%% val cap, max_val_tokens={opts.max_val_tokens:,d}): "
        f"{train_tokens:,d} / {val_tokens:,d} tokens"
    )
...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=int,
        default=5_000_000,
        help="Validation reserve subtracted from train budget (train ≈ max_tokens minus this); "
        "actual val size is min(20%% of --max-tokens, this cap).",
    )
