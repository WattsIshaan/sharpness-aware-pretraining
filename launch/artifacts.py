...
from launch.globals import get_remote_files
import math
...
LIST_OF_VAL_FILES = {
    ...
    #"starcoder-validation": ['datasets/starcoder/val/eval-data_perplexity_v3_small_gptneox20b_starcoder_en_val_part-0-00000.npy']
}
...
class ModelEvaluation(Artifact):
    ...
    def construct(self, builder: Task):
        ...
        random_string = ''.join([random.choice('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz') for _ in range(8)])
        local_root = os.path.join(LOCAL_DATA_PATH, random_string)
