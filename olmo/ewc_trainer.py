...
main CPT iterator is not advanced). If ``ewc_fisher_paths`` is set (e.g. subsampled pretrain
**train** memmaps), those are used; otherwise the CPT training paths are used.
"""
...
class EWCTrainer(Trainer):
    ...
    def prepare_ewc(self) -> None:
        ...
        if cfg.ewc_fisher_paths:
            # Fisher data often uses different memmaps than CPT training. Both dataloaders default
            # to ``save_folder / "train_data" / "global_indices.npy"``; reusing the same folder
            # would overwrite CPT indices and break the main train loader. Use an auxiliary folder.
            ...
            log.info(
                "EWC: estimating Fisher diagonal over %d batches (λ=%s) on CPT training data...",
                cfg.ewc_fisher_batches,
                cfg.ewc_lambda,
            )
